...
grab_lodes <- function(
  ...
  if (length(state) > 1 || length(year) > 1) {
    ...
    results <- apply(state_year, 1, function(df_row) {
      grab_lodes(
        df_row[1],
        df_row[2],
        version = version,
  ...
  if (!is.logical(use_cache)) {
    stop("The use_cache paramater must be either TRUE or FALSE")
  }
  ...
  if (lodes_type == "od" && !(state_part %in% c("main", "aux"))) {
    state_part <- "main"
    warning(
      "'state_part' is required when setting lodes_type =\"od\", defaulting to state_part=\"main\""
    )
  }
  ...
  if (lodes_type == "od") {
    # Set url for od
    url <- glue::glue(
      "https://lehd.ces.census.gov/data/lodes/{version}/{state}/{lodes_type}/{state}_{lodes_type}_{state_part}_{job_type}_{year}.csv.gz"
    )
    ...
    # Set url for rac/wac
    url <- glue::glue(
      "https://lehd.ces.census.gov/data/lodes/{version}/{state}/{lodes_type}/{state}_{lodes_type}_{segment}_{job_type}_{year}.csv.gz"
    )
  ...
  if (use_cache) {
    ...
    if (file.exists(fil)) {
      ...
      if (lodes_resp$status_code >= 400) {
        ...
        rlang::abort(c(
          glue::glue(
            "lehdr: Connection error while downloading: {lodes_req$url}"
          ),
          "i" = "Empty response."
        ))
      } else {
    ...
    # User did not allow cache to be used
    if (file.exists(fil)) {
    ...
    rlang::inform(glue::glue("Downloading {url} to {basename(fil)} now..."))
